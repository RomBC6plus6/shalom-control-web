...
def cargar_datos(archivo):
    ...
    df["F.SALIDA_DT"] = pd.to_datetime(df["F.SALIDA"], errors="coerce")
...
def filtrar_datos(df, dias_min, dias_max):
    ...
    # Calcular días enteros transcurridos
    df["DIAS_EN_ALMACEN"] = (fecha_hoy - df["F.SALIDA_DT"]).dt.days
    ...
    # Ordenar de mayor a menor días retenido para mayor orden
    df_filtrado = df_filtrado.sort_values(by="DIAS_EN_ALMACEN", ascending=False)
    return df_filtrado
...
def generar_notificaciones(df_alertas):
    ...
    for _, fila in df_alertas.iterrows():
        ...
        fecha_salida = (
            fila["F.SALIDA_DT"].strftime("%Y-%m-%d") 
            if pd.notnull(fila["F.SALIDA_DT"]) 
            else str(fila.get("F.SALIDA", ""))
        )
...
dias_minimos = st.sidebar.slider(
    "Días mínimos retenido:", min_value=1, max_value=60, value=7
)
